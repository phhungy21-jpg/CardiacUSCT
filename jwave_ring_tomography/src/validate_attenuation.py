"""Validate attenuation_solver.py against the known analytic absorption
law, in THIS project's own domain/context -- `jwave_test`'s validation
of the same code does not carry over (different domain, different
background medium); a physics addition needs its own check here.

Uses MYOCARDIUM (not chest-wall-proxy, unlike jwave_test's original
validation) since that's the tissue directly relevant to the inner-
boundary bias question this validation is being done to investigate.

Method (same as jwave_test): homogeneous medium, point source, several
receivers at increasing distance. Run BOTH the lossless jWave solver
and the new attenuating solver on the IDENTICAL geometry. The ratio
(attenuating peak / lossless peak) at each receiver cancels the common
geometric-spreading factor, isolating the pure absorption factor
exp(-alpha*(r-r_ref)) -- directly checkable against the cited
...
tissue = cfg.MYOCARDIUM
sound_speed_field = FourierSeries(
    jnp.full((*N, 1), tissue.sound_speed, dtype=jnp.float32), domain)
medium = Medium(domain=domain, sound_speed=sound_speed_field, density=tissue.density)
...
atten_field = FourierSeries(jnp.full((*N, 1), alpha_np_per_m, dtype=jnp.float32), domain)
...
def run_lossy(medium, p0):
    return simulate_wave_propagation_attenuating(medium, time_axis, atten_field, cfg.F0_HZ, p0=p0)
