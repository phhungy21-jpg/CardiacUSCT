"""Phase 3 — multistatic backprojection on a beating TRIANGLE, 8-probe
variant (2 probes per wall, at the 1/3 and 2/3 points).

Direct fix for the failure diagnosed in run -29
(phase3_multistatic_backprojection_triangle.py, 4-probe version): with
one probe at each wall's exact MIDPOINT, the top and bottom probes (and
left/right) sit exactly antipodal through the domain center, so that
tx/rx pair's constant-round-trip-time locus degenerates to a straight
line lying right on the very axis being peak-searched -- it swamped the
genuinely weak vertex/corner echo. Per user's suggestion: split each
wall's single probe into TWO, at the 1/3 and 2/3 points along the wall
(not the midpoint), giving 8 probes / 64 tx-rx pairs total. No probe now
sits exactly on the vertex axis (col=center), so no single pair's
locus can degenerate onto exactly that search line, and the fraction of
any remaining exactly-antipodal pairs drops relative to genuinely
localizing ones (4 possible antipodal pairs out of 64, vs 2 of 16
before). Otherwise identical to run -29: same triangle phantom, same
accumulation machinery (envelope detection, direct-arrival exclusion,
group-delay correction), same per-axis ground-truth formulas
(top=R, bottom=R/2, left=right=R/sqrt(3)), same search axes (the
ground truth axes are properties of the TRIANGLE, not the probes, so
they don't change).
"""
...
THIRD_OFFSET_CELLS = PROBE_DIST_CELLS // 3  # 1/3 and 2/3 points along each wall

PROBES = {
    "top_a":    dict(row=center[0] - PROBE_DIST_CELLS, col=center[1] - THIRD_OFFSET_CELLS, axis="col"),
    "top_b":    dict(row=center[0] - PROBE_DIST_CELLS, col=center[1] + THIRD_OFFSET_CELLS, axis="col"),
    "bottom_a": dict(row=center[0] + PROBE_DIST_CELLS, col=center[1] - THIRD_OFFSET_CELLS, axis="col"),
    "bottom_b": dict(row=center[0] + PROBE_DIST_CELLS, col=center[1] + THIRD_OFFSET_CELLS, axis="col"),
    "left_a":   dict(row=center[0] - THIRD_OFFSET_CELLS, col=center[1] - PROBE_DIST_CELLS, axis="row"),
    "left_b":   dict(row=center[0] + THIRD_OFFSET_CELLS, col=center[1] - PROBE_DIST_CELLS, axis="row"),
    "right_a":  dict(row=center[0] - THIRD_OFFSET_CELLS, col=center[1] + PROBE_DIST_CELLS, axis="row"),
    "right_b":  dict(row=center[0] + THIRD_OFFSET_CELLS, col=center[1] + PROBE_DIST_CELLS, axis="row"),
}
...
if __name__ == "__main__":
    ...
    print("Multistatic backprojection on a beating TRIANGLE: 8-probe "
          "(2 per wall, at the 1/3 and 2/3 points), 64-pair accumulator -- "
          "fix attempt for run -29's colinear-pair vertex-axis artifact. "
          "vertex (top), normal-incidence edge (bottom), oblique facets "
          "(left/right) all in one phantom.")
    ...
    plt.savefig("results/figures/phase3_multistatic_backprojection_triangle_8probe.png", dpi=130)
    print("\nSaved results/figures/phase3_multistatic_backprojection_triangle_8probe.png")
